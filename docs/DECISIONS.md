...
### D8: 互动率自动计算（对 PRD F3「不自动计算比率」的显式修订）

- 决策变化：用户（工具唯一操作者）明确要求自动计算互动率。原保守立场基于 YouTube 派生指标政策——该政策约束的是**对外分发的产品**，单人本地工具仅展示参考值、不生成评分/排名，政策风险不适用
- 实现口径：缓存仍只存原始值（view/like/comment），互动率由前端展示层按次计算——**赞/观看 ≥5%、评/赞 ≥10% 绿显**（阈值来自实习 SOP「点赞/观看≥5%、评论/点赞≥10%」），不做综合评分、不排序、不淘汰
- 除数为 0 或字段缺失 → 显示「未知」，不算 0（延续空值口径）
- 同批新增字段：频道总播放量/公开视频数（channels.list statistics，34 字段表「播放量评估」数据源）、视频 ID（构造 watch 链接，修复 PRD §4.2「视频链接」缺口）、视频描述（竞品植入线索；前端对 sponsor/Sponsored/#品牌tag 等关键词打「疑似商业植入」徽章，仅提示由人工核对）
- 旧缓存行无新字段时优雅降级显示「未知」/无链接；demo 合成种子不补造链接（demo: 前缀不伪装真实频道）

## 2026-09-11 · 方案A：Nox CSV 桥（D9）

- 定位：与 Nox/Modash 等红人数据工具是**上下游互补**而非集成——它们管「发现与数据」，本工具管「判断留存与行动衔接」；不做 API 直连（企业定价、黑箱口径、违背单后端约束）
- 桥接语义：工具导出的**结论/估算是判断依据，不是原始数据**——列名自动识别（中英变体）后构建为 `source_note` 来源备注（含工具标签+日期），进候选档案与 F5 导出，不进 api_cache 原始缓存（避免污染「可防守依据」）
